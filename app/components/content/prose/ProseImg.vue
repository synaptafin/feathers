<template>
  <figure>
    <component
      :is="imgComponent" :src="refinedSrc" :alt="alt" :width="width" :height="height"
      :aria-labelledby="alt ? `caption-${uniqueId}` : undefined" />
    <figcaption
      v-if="alt" :id="`caption-${uniqueId}`"
      class="text-sm text-center text-gray-600 dark:text-gray-300 mt-1">
      {{ alt }}
    </figcaption>
  </figure>
</template>
